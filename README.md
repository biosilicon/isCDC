...
不纳入版本控制，仅在需要委派时准备相应角色；缺少本地角色时可由主 agent 按同一流程处理。

Agent 从 [AGENTS.md](AGENTS.md) 获取工作边界、环境和按任务划分的文档入口。
[开发约束](doc/开发约束.md) 保存功能专属契约和回归覆盖要求，按本次改动查阅相关章节。
普通文档修订以链接和内容一致性检查为主；代码修改完成实现、相关验证及所引入问题的修复后
交付。已有授权内的可逆工作和相关测试重跑无需逐步确认。维护 agent 指令或项目 skill 时，
只保留会改变决策的项目知识，使用简短、准确的触发描述，将条件性细节放到可按需阅读的资料中；
避免固定报告模板、无条件全库阅读和重复批准。指令应适用于贡献者使用的不同模型，
模型选择和并发配置按实际任务另行决定。
...
最窄的相关测试文件；相关检查通过后不无故扩大或重复测试，不自动运行完整套件。例如：
...
本次任务指定的 `temp/` 数据集按五个阶段处理；允许委派且有独立工作时使用两种 agent 角色：

1. 主 agent 或只读的 `dataset_planner` 调查指定数据集，形成有证据的处理方案。
2. 复用已有用户决定并定稿，只将新增实质选择或缺失授权提交用户；遵守用户指定的审阅门槛。
3. 主 agent 或 `dataset_worker` 按已获授权的最终计划在隔离目录转换、验证并实际重放。
4. 主 agent 独立回查来源、文件内容、元数据和验证报告，修复既定规则内的失败。
5. 请求包含入库时，由主 agent 对验收通过的产物串行导入并完成后续检查。

各数据集可按依赖推进，无需等待整批规划或转换结束；共享决策和最终 Challenge 排名仍须
在相应依赖齐备后处理。原始输入和可复现材料持续保留；清理须有具体范围的授权并先完成归档。
...
完整的并发数、路径隔离、授权边界、完成标准、验收和清理规则见

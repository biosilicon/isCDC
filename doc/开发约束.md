...
Hide features from search only when every observed value in the full Database modality equals
zero. Check actual values, including explicitly stored sparse zeros; neither sparse entry count
nor column sums establish expression. Preserve negative, cancelling, non-finite and unobserved
features. Apply filtering before exact/alias/prefix matching and pagination, retain original column
keys, and prefer the first searchable feature by default. An all-zero modality has an empty search
index and retains a valid original feature for its zero-valued view.
...
Existing generations may receive an independent filtered-search publication bound to the exact
parent generation and manifest hash. Audit the prepared matrices and index hashes offline, retain
the unmodified primary publication and values, copy verified indices, and switch one search pointer
atomically. Ignore overlays for a replaced parent generation; do not perform filtering at startup
or in a search request. New full preparations include the filter directly in their indices.
